...
export const MessageInput: React.FC<{ conversationId: number }> = ({ conversationId }) => {
  ...
  return (
    <form onSubmit={handleSend} className="flex flex-col w-full relative">
      ...
      <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-full px-2 py-2 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-transparent transition-all">
        <button 
          type="button" 
        ...
        <input
          ...
          placeholder="Message..."
          className="flex-1 bg-transparent border-none focus:ring-0 text-[15px] py-1.5 px-2 text-slate-900 dark:text-white placeholder-slate-500"
          disabled={uploading}
        />
